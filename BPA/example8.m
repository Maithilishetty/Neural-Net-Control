...
for k = 1:length(n)
        ...
        yp1 =(5*yp(end)*yp(end-1)/(1 + (yp(end))^2 + (yp(end-1))^2+ (yp(end-2))^2))+ u(end)+0.8*u(end-1);
        yp=[yp yp1];
        ...
        v3 = w3*y2_k; 
        y3 = v3 ;
        E  = (5*yp(end-1)*yp(end)/(1 + (yp(end))^2 + (yp(end-1))^2+ (yp(end-2))^2)) - y3;
        phi_v3_diff = (1);
        ...
        u1 = (-y3 -(0.8)*u(end)+0.32*yp(end) + 0.64*yp(end-1)-0.5*yp(end-2)+ r);
...
title('Example 8 with control - Two Layer BPA');
